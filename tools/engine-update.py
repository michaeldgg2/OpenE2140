...
def download_file_from_github(owner: str, repo: str, branch: str, file_path: str, gh_token: str = "") -> Optional[str]:
	url = f"https://raw.githubusercontent.com/{owner}/{repo}/refs/head/{branch}/{file_path}"
	cmd = ["curl", "-s", "-f"]
	if gh_token:
...
def main() -> None:
	...
	# If local mod.config already points to latest engine commit, it means the engine was already updated.
	# Try to close any open PR and exit.
	...
	if downloaded is not None:
		...
		if current_engine_version and current_engine_version == engine_commit_hash:
			...
			should_update_mod_config = False
	else:
		raise RuntimeError(f"Could not download mod.config from remote branch {branch}.")

	if should_update_mod_config:
